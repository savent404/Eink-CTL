...
//画笔颜色,背景颜色
unsigned short POINT_COLOR = 0x0000,BACK_COLOR = 0xFFFF;  
...
{
	static GPIO_InitTypeDef outpute={
		.Pin = GPIO_PIN_All,
...
//Xpos:横坐标
//Ypos:纵坐标
__inline void LCD_SetCursor(unsigned short Xpos, unsigned short Ypos)
{
#if USE_HORIZONTAL==1 
...
//	GPIO_SetBits(GPIOE, GPIO_Pin_15 | GPIO_Pin_14);
...
//num:数值(0~99);	 
void LCD_Show2Num(unsigned short x,unsigned short y,unsigned short num,unsigned char len,unsigned char size,unsigned char mode)
{         	
	unsigned char t,temp;						   
	for(t=0;t<len;t++)
	{
...
void LCD_ShowUsrFont24(unsigned short x, unsigned short y, const char *src) {
  ...
  for (line = 0; line < 24; line ++) {
    ...
    for (i = 0; i < 8; i++) {
      if (buff & 0x01) {
        LCD_DrawPoint(i + 16 + x, line+ y);
      }
      buff >>= 1;
    }
  }
}
void LCD_ShowUsrFont16(unsigned short x, unsigned short y, const char *src) {
  int line, i;
  unsigned char buff;
  const unsigned char *pt = (const unsigned char*)src;
...
//每次刷新120行，总共刷新5次，刷新完一副图像
void LCD_ShowPicture(unsigned short x_start,unsigned short y_start,unsigned char* pic)
{
	unsigned short num=0, pic_line, pic_column;
	
	
	for (pic_line=0;pic_line<120;pic_line++){
			for (pic_column=0;pic_column<200;pic_column++){
				
				// 低8位*2 = R
				POINT_COLOR=(((pic[num]&0x03)+(pic[num]>>2&0x03)+(pic[num]>>4&0x03)+(pic[num]>>6&0x03))*2<<11)
				
				// 低8位*4 = G
				+(((pic[num]&0x03)+(pic[num]>>2&0x03)+(pic[num]>>4&0x03)+(pic[num]>>6&0x03))*4<<5)
				
				// 低8位*2*4 = B
				+(((pic[num]&0x03)+(pic[num]>>2&0x03)+(pic[num]>>4&0x03)+(pic[num]>>6&0x03))*2);

					num++;
					LCD_DrawPoint(pic_column + x_start,pic_line/4 + y_start);
			}
	}
}
void LCD_Inverse_Color(unsigned short x_start,unsigned short y_start,unsigned short x_size,unsigned short y_size)
{
	unsigned short i,j;
	for (i=0;i<x_size;i++){
				for (j=0;j<y_size;j++){
						
						if(LCD_ReadPoint(i+x_start,j+y_start)==WHITE)	POINT_COLOR = RED;
						else POINT_COLOR = LCD_ReadPoint(i+x_start,j+y_start);
						LCD_DrawPoint(i+x_start,j+y_start);
				}
	}
}	
